...
## improved_v2 finalization layer

This implementation extends `improved` with:

- Leakage-aware final ensemble validation using explicit split annotations when available.
- Diagnostic compound/scaffold/source-group splits when explicit splits are unavailable.
- Common-test evaluation of all Stage 1/2/3 score columns on the same pairs.
- Calibration metrics: Brier score, expected calibration error, and calibration bins.
- Uncertainty ranking from base-model disagreement and probability margin.
- Per-CYP/per-target metrics and per-target meta-model diagnostics.
- Multi-seed final ensemble aggregation.
- External validation hook via `--external-labels`.
- Candidate ranking outputs per target.
- HPO/seed sweep command generation through `python -m pring_modeling.hpo_plan`.

Run all finalized models with:

```bash
MODEL_IMPL=improved_v2 RUN_FINAL_VALIDATION=true sbatch modeling/scripts/run_all_models_compare_hpc.sh
```
