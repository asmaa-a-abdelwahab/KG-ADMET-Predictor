...
def _runtime_compatibility(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if not expected:
        return {
            "status": "unverified_legacy_artifact",
            "expected": {},
            "actual": actual,
            "mismatches": {},
        }
...
def _verify_artifact_digest(path: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    expected = clean_optional_text(manifest.get("model_artifact_sha256"), "")
    if not expected:
        return {"status": "unverified_legacy_artifact", "expected": None, "actual": None}
    digest = hashlib.sha256(path.read_bytes()).hexdigest()
    if digest != expected:
        raise RuntimeError(
            f"Serialized model digest mismatch for {path.name}: "
            f"expected {expected}, calculated {digest}"
        )
...
class PRINGPredictionService:
    ...
    def __init__(self) -> None:
        ...
        if require_verified_artifacts and (
            ...
            raise RuntimeError(
                "Production serving requires a manifest with runtime versions and a verified "
                "model artifact digest. Rebuild the production bundle."
            )
        if (
            self.runtime_compatibility["status"] == "incompatible"
            and not _env_flag("PREDICTION_ALLOW_RUNTIME_MISMATCH", False)
        ):
            raise RuntimeError(
                "Prediction runtime does not match the serialized model environment: "
                f"{self.runtime_compatibility['mismatches']}"
            )
        ...
        self.fallback_artifact_integrity: dict[str, Any] = {"status": "not_configured"}
        ...
        if fallback_file.exists():
            ...
            if (
                ...
                raise RuntimeError(
                    "Stage 3 fallback runtime does not match its serialized environment: "
                    f"{fallback_compatibility['mismatches']}"
                )
        ...
        self.allow_stale_cache = _env_flag("PREDICTION_ALLOW_STALE_CACHE", False)
        ...
        return {
            ...
            "live_model_variants": {
                ...
                "stage3_fallback": {
                    ...
                    "artifact_integrity": self.fallback_artifact_integrity,
    ...
    def _lookup_scores(self, pair: ResolvedPair) -> tuple[dict[str, float], dict[str, Any]] | None:
        ...
        if reference_row is not None:
            return self._scores_from_row(reference_row), {
                ...
                "reference_provenance_audit": self.reference.provenance_audit,
            }
        model_versions = {
            "primary": clean_optional_text(self.bundle.get("model_version"), "unknown_model_version"),
        }
        if self.fallback_bundle is not None:
            model_versions["stage3_fallback"] = clean_optional_text(
                self.fallback_bundle.get("model_version"), "unknown_model_version"
            )
        cache_row = self.cache.get(
            pair.compound_key,
            pair.target_key,
            model_versions=model_versions,
            graph_version=self.graph_version,
            allow_stale=self.allow_stale_cache,
        )
    ...
    def _evidence(self, pair: ResolvedPair) -> dict[str, Any]:
        ...
        CALL (c, p) {
          ...
          OPTIONAL MATCH (bioassay:BioAssay)-[:HAS_MEASURE_GROUP]->(measure_group)
          WITH direct,
        ...
        direct_labels = {
            str((item.get("interaction") or {}).get("label", "")).strip().lower()
            for item in direct
        }
        has_active_assertion = bool(direct_labels & {"curated_active", "active", "positive"})
        has_inactive_assertion = bool(
            direct_labels & {"curated_inactive", "inactive", "inactive_or_weak", "negative"}
        )
        if "curated_conflicting" in direct_labels or (has_active_assertion and has_inactive_assertion):
            direct_assertion_state = "conflicting"
        elif has_active_assertion:
            direct_assertion_state = "active"
        elif has_inactive_assertion:
            direct_assertion_state = "inactive"
        elif direct:
            direct_assertion_state = "unlabeled"
        else:
            direct_assertion_state = "absent"
        ...
        return {
            ...
            "direct_assertion_state": direct_assertion_state,
            "direct_assertion_labels": sorted(label for label in direct_labels if label),
    ...
    def _result_classification(predicted_label: int, evidence: dict[str, Any]) -> tuple[str, str]:
        ...
        direct_state = str(evidence.get("direct_assertion_state", "active_or_unspecified"))
        if direct_state == "inactive" and predicted_label:
            return "prediction_conflicts_with_known_inactive", "Prediction conflicts with a curated inactive assertion"
        if direct_state == "inactive" and not predicted_label:
            return "known_inactive_consistent", "Model result is consistent with a curated inactive assertion"
        if direct_state == "conflicting":
            return "known_evidence_conflict", "Curated evidence is conflicting; prediction requires adjudication"
    ...
    def _recommended_action(result_status: str, evidence: dict[str, Any], model_certainty: dict[str, Any]) -> str:
        ...
        if result_status == "prediction_conflicts_with_known_inactive":
            return "Do not present this as novel. Review the curated inactive evidence, label policy, and assay context before use."
        if result_status == "known_inactive_consistent":
            return "Treat this only as consistency with the aggregated inactive label, not proof of biological inactivity."
        if result_status == "known_evidence_conflict":
            return "Adjudicate the conflicting endpoint evidence and report the conflict explicitly; do not use this pair as a clean label."
    ...
    def _build_result(
        ...
        reference_audit = self.reference.provenance_audit
        legacy_diagnostic_warning = reference_audit.get("warning")
        return _json_safe(
            {
                "pair": asdict(pair),
                "model": {
                ...
                "validation": {
                    ...
                    "publishable": variant_manifest.get("publishable", self.manifest.get("publishable")),
                    "reference_provenance_audit": reference_audit,
                },
                "interpretation": {
                    "statement": statement,
                    "model_certainty": certainty["band"],
                    ...
                    "scope_warning": (
                        ...
                        + (f" Scientific-validity warning: {legacy_diagnostic_warning}" if legacy_diagnostic_warning else "")
                    ),
                },
            }
        )
            ...
            rows.append(
                {
                    ...
                    **{column: float(scores.get(column, np.nan)) for column in self.score_columns},
                    ...
                    "split_registry_id": bundle.get("split_registry_id"),
                    "feature_schema_id": bundle.get("feature_schema_id"),
                    "label_policy_id": bundle.get("label_policy_id"),
        ...
        return _json_safe({
            ...
            "report_context": {
                "model_provenance": {
                    ...
                    "score_columns": self.score_columns,
                    "publishable": self.manifest.get("publishable"),
                    "reference_provenance_audit": self.reference.provenance_audit,
