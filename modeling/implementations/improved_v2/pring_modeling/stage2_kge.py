...
def train_supervised_kge_decoder(model, dataset: KGDataset, modeling_root: Path, relation_name: str, device, args: argparse.Namespace, out_dir: Path) -> dict:
    ...
    threshold, valid_metrics = optimize_binary_threshold(
        y_valid,
        valid_score,
        metric=args.supervised_threshold_selection,
        min_specificity=args.supervised_min_specificity,
        min_recall=args.supervised_min_recall,
    )
    ...
    operating_points = operating_point_metrics(
        ...
        test_score,
        binary_metrics,
        min_specificity=max(args.supervised_min_specificity, args.report_min_specificity),
    ...
    per_target_metrics_file = write_dataframe_if_not_empty(per_target, out_dir / "supervised_per_target_metrics.csv")
    ...
    return {
        ...
        "class_distribution": {
            ...
            "valid": class_distribution(y_valid),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--supervised-threshold-selection", choices=["mcc", "balanced_accuracy", "youden", "f1", "accuracy", "recall", "specificity"], default="mcc")
    parser.add_argument("--supervised-min-specificity", type=float, default=0.0, help="Optional validation threshold constraint for the supervised decoder.")
    parser.add_argument("--supervised-min-recall", type=float, default=0.0, help="Optional validation recall constraint for the supervised decoder.")
    ...
    parser.add_argument("--target-group-column", default="auto", help="Column used for per-CYP/per-target decoder metrics.")
